...
public class Lexer {
  ...
  public static void main(String args[]) {
    Token token;
    ArrayList<String> linesPrintOut = new ArrayList<>();
    int lineCopied = 0;
    ...
    try {
      ...
      while (token != null) {
        ...
        if (lineCopied < lex.getLineFound()) {
          linesPrintOut.add(lex.getLastLine());
          lineCopied = lex.getLineFound();
        }
        System.out.format("%-11s left: %-8d right: %-8d line: %-8d %s\n",
            token.toString(), token.getLeftPosition(),
            token.getRightPosition(), token.getLineNumber(),
            token.getKind());
        token = lex.nextToken();
      }
      if (lineCopied < lex.getLineFound()) {
    ...
    for (int i = 0; i < linesPrintOut.size(); i++) {
      System.out.format("%3d: %s\n", i + 1, linesPrintOut.get(i));
    }
